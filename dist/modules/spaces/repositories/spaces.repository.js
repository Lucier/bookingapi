...
let SpacesRepository = class SpacesRepository {
    ...
    async create(data) {
        const [space] = await this.db.insert(index_1.spaces).values(data).returning();
        return space;
    }
    findAll() {
        return this.db.select().from(index_1.spaces);
    }
    async findById(id) {
        const [space] = await this.db.select().from(index_1.spaces).where((0, drizzle_orm_1.eq)(index_1.spaces.id, id));
        return space;
    }
    async update(id, data) {
        const [updated] = await this.db
            .update(index_1.spaces)
            .set({ ...data, updatedAt: new Date() })
            .where((0, drizzle_orm_1.eq)(index_1.spaces.id, id))
            .returning();
        return updated;
    }
    async delete(id) {
        await this.db.delete(index_1.spaces).where((0, drizzle_orm_1.eq)(index_1.spaces.id, id));
    }
};
